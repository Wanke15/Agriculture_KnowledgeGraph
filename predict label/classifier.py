...
class Classifier:
    ...
    def similarity(self, item1, item2):  # 比较两个页面的相似度，返回[-1,1]之间的相似度
        ...
        # 各组相似度线性加权
        simi = self.weight[0] * title_simi + self.weight[1] * openTypeList_simi + self.weight[
            2] * baseInfoKeyList_simi + self.weight[3] * baseInfoValueList_simi
    ...
    def KNN_predict(self, item):  # 预测互动页面的类别
        ...
        for p in self.labeled_hudongList:  # 计算各项相似度的加权和
            s = self.weight[0] * title_simi[i] + self.weight[1] * openTypeList_simi[i] + self.weight[2] * \
                baseInfoKeyList_simi[i] + self.weight[3] * baseInfoValueList_simi[i]
            count += 1
            if count < 2:
                pass
